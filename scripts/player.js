...
export class Player {
   ...
   */
  onKeyDown(event) {
    if (window.__onboarding) return; // typing name/photo — don't grab the pointer
   ...
   */
  onMouseDown(event) {
    // While the rifle is equipped, the weapon handles mouse input (shoot/aim).
    if (window.__weaponActive) return;
    if (this.controls.isLocked) {
      // Is a block selected?
