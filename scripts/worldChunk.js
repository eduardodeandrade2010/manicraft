...
import { generateStructures as runStructures } from './structures';
...
export class WorldChunk extends THREE.Group {
  ...
  */
      ...
      for (let z = 0; z < this.size.width; z++) {
        ...
        for (let y = this.size.height; y >= 0; y--) {
          if (y <= this.params.terrain.waterOffset && y === height) {
            ...
            if (y > 2 && y < height - 1) {
              const cave = simplex.noise3d(
                (this.position.x + x) / 24,
                y / 14,
  ...
  generateWater() {
    this.add(createWaterMesh(this.size.width, this.params.terrain.waterOffset));
  }
  ...
  /**
   * Returns the first free cell above the terrain surface at local (x,z),
   * ignoring clouds. Used by the structure generator. Null if no ground.
   */
  columnSurface(x, z) {
    for (let y = this.size.height - 2; y >= 0; y--) {
      const b = this.getBlock(x, y, z);
      if (b && b.id !== blocks.empty.id && b.id !== blocks.cloud.id) return y + 1;
    }
    return null;
  }
